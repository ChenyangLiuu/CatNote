...
public class NoteAdapter extends BaseAdapter implements Filterable {
    ...
    private List<com.example.catnote.Note> backList;//用来备份原始数据
    private List<com.example.catnote.Note> noteList;//这个数据是会改变的，所以要有个变量来备份一下原始数据
    private MyFilter mFilter;

    public NoteAdapter(Context mContext, List<com.example.catnote.Note> noteList) {
    ...
    class MyFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence charSequence) {
            ...
            List<com.example.catnote.Note> list;
            if (TextUtils.isEmpty(charSequence)) {//当过滤的关键字为空的时候，我们则显示所有的数据
                list = backList;
            } else {//否则把符合条件的数据对象添加到集合中
                list = new ArrayList<>();
                for (com.example.catnote.Note note : backList) {
        ...
        protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
            noteList = (List<com.example.catnote.Note>)filterResults.values;
